...
private final class AnnotationCanvasOverlayView: NSView {
    ...
    private func drawOutOfCapturePattern(excluding imageRect: CGRect) {
        let settings = controller.outOfCapturePatternSettings
        guard settings.isEnabled else {
            return
        }

        for rect in outOfCaptureRects(excluding: imageRect) where rect.width > 0 && rect.height > 0 {
            drawOutOfCapturePattern(in: rect, settings: settings)
        }
    }

    private func outOfCaptureRects(excluding imageRect: CGRect) -> [CGRect] {
    ...
    private func drawOutOfCapturePattern(in rect: CGRect, settings: EditorOutOfCapturePatternSettings) {
        NSGraphicsContext.saveGraphicsState()
        ...
        let palette = outOfCapturePatternPalette()
        ...
        for row in minDotRow...maxDotRow {
            for column in minDotColumn...maxDotColumn where (row + column).isMultiple(of: 2) {
                ...
                let dotRect = CGRect(x: dotPoint.x - dotRadius, y: dotPoint.y - dotRadius, width: settings.dotDiameter, height: settings.dotDiameter)
                NSBezierPath(ovalIn: dotRect).fill()
            }
        }
    }

    private func outOfCapturePatternPalette() -> (background: NSColor, line: NSColor, dot: NSColor) {
