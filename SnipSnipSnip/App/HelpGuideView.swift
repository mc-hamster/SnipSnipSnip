                ...
                HelpArticle(
                    ...
                    sections: [
                        ...
                        HelpArticleSection(
                            ...
                            body: "Choose Interactive HTML for a portable single-file composition that works offline. A determinate progress panel shows the current export stage and offers Cancel Export while the embedded images are prepared and saved. Step navigation and comparison controls never contact a server. A Comparison starts with the app’s chosen view; use Compare Using to switch among Side by Side, Wipe, Overlay, Blink, Difference, and Highlight Changes. Drag directly on Wipe, use Fit or Zoom for synchronized inspection, and bookmark the current file URL when you want its fragment to restore the viewer settings. SnipSnipSnip losslessly optimizes embedded PNG pixels at their full dimensions without source metadata, escapes user-authored text, and restricts the file with a deny-by-default Content Security Policy whose only executable resources are the exact exporter-owned style and interaction code. A subtle line-and-dot background and small embedded app logo brand the outer page without covering the composition. The pattern is removed for print and Increased Contrast. The linked attribution makes no network request unless chosen."
                        ),
                        HelpArticleSection(
