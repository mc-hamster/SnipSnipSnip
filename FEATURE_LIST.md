...
- **SnipSnipSnip**: the App Store local-first screenshot and screen recording app. It includes region, window, frontmost-window, fullscreen, repeat, and timer screenshot capture; the screenshot editor; shared multi-capture Composition, Compare, Steps, and interactive HTML; archive/history/recovery; Clipboard History; Screen Inspector; floating references; local export and sharing; and region, window, and fullscreen screen recording. It can open, edit, save, recover, and export existing `.sssguide` documents, but cannot create or record new Guides.
...
- Intent-driven screenshot creation with durable Screenshot, Comparison, Steps, and Collection purposes; contextual Capture After, Capture Next Step, and Add Image actions; reusable layouts; independent item and whole-result annotations; and offline interactive HTML.
...
| Composition output | ✓ Done | Plain and Styled PNG/JPEG/PDF share the composition render pipeline; oversized rasters are preflighted with Scale to Fit or streamed one-step-per-page PDF choices. Steps also supports a reusable Steps per Page PDF setting. Blink static output defaults to After and deterministic GIF/APNG/MP4 preserves interval, crossfade, and loop choices with a disclosed 4,096 px animated cap. Compare and Steps export as one offline interactive HTML file with exact rendered Difference/Change Highlight results, custom or hidden step numbering, print/no-script fallbacks, metadata-stripped pixels, no source paths or capture metadata, no remote resources, and a hash-pinned deny-by-default Content Security Policy. | Keep raster-size, animation determinism, browser, VoiceOver, keyboard, reduced-motion, and large-file guardrail coverage in the release matrix. |
...
- Intent-driven screenshot creation and multi-capture composition are shipped for static and interactive output, including Comparison, manual Steps, Collection layouts, optional Polish Looks and Mockups, user templates, WYSIWYG output, and offline HTML.
...
5. Add conflict detection and keyboard-capture UX polish for customizable global hotkeys.
...
SnipSnipSnip is already a strong local-first screenshot product with a meaningful editor, archive system, multi-capture composition, presentation styling, local drag-out sharing, and first-generation automation across App Intents, CLI, AppleScript, and URL routes. It also has a real, useful first-generation recording stack. SnipSnipSnip Pro extends that product with advanced capture workflows: scrolling capture, connected iPhone/iPad screenshot capture, and UI Map capture. The remaining premium-suite gaps center on advanced video polish, deeper integrations, Pro capture hardening, and support readiness.
