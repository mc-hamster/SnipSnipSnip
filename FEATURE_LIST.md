...
- **SnipSnipSnip**: the standard local-first screenshot, Guide, and screen recording app. It includes region, window, frontmost-window, fullscreen, repeat, and timer screenshot capture; the screenshot editor; editable workflow Guides; archive/history/recovery; Clipboard History; Screen Inspector; floating references; local export and sharing; and region, window, and fullscreen screen recording.
...
- Editable Guides for Window, App, single-display Region, and Display workflows, with automatic action steps, optional source video/audio, local captions, recovery, non-destructive editing, branding, and document, animation, video, image, and ZIP exports.
...
| --- | --- | --- | --- |
| First-class Guide workflow | ✓ Done | Guide sits beside Screenshot and Record, supports Window, App, single-display Region, and Display setup, automatically follows moving/resized app windows across mixed-scale and rotated displays, and includes a noncapturable HUD, pause/resume, Manual Step, Undo Last, recovery, and a dedicated three-pane editor. | Continue signed-sandbox compatibility testing across third-party apps and transient system UI. |
...
| Guide export | ✓ Done | PDF, Word, GIF, APNG, PNG/JPEG step images, ZIP/Markdown, Full Motion MP4, Action Highlights MP4, and Step Slideshow MP4 use shared rendering/media helpers, honest step/segment/encoder/entry progress, disk-headroom checks, incremental rendering, streaming ZIP64, cancellation, and atomic output replacement. ZIP is all-or-nothing across selected nested formats. Source-video MP4s apply track orientation and add a smooth custom cursor, trail, and click highlights. Missing source media disables the two dependent MP4 modes with an explicit explanation. | Continue visual snapshot coverage on unusual third-party display/codec combinations. |
| Guide automation | ✓ Done | The bundled CLI helper, v1 URL routes, AppleScript, and App Intents share commands for start, pause, resume, Manual Step, stop, open `.sssguide`, and export, with Guide-specific payloads and errors. URL-triggered Guide export uses Downloads. | URL routes remain trigger-oriented. Sandboxed App Store automation restricts unattended file destinations to Downloads; interactive exports can use user-selected locations. |
...
| Settings window | ✓ Done | The app has General, Presets, Shortcuts, Recording, Archive, Clipboard, and Privacy settings tabs. General includes Screenshot Capture, Export & Sharing, and other everyday preferences. Presets manages saved screenshot capture setups. Shortcuts centralizes global capture hotkey customization, the single-key editor shortcut toggle, and the shortcut reference. In SnipSnipSnip Pro, General settings include Window UI Map enablement and default pinned UI Map overlay options; Clipboard settings include history enablement, item/storage limits, clear history, ignored-app management, and restore-default ignored apps. | Deeper workflow automation settings are still limited. |
...
| App Shortcuts and App Intents | ✓ Done | `SnipSnipSnip/Automation/AppIntents` registers native App Intents over the shared automation service for status, preset listing/running, fullscreen capture, frontmost-window capture, fixed or interactive region capture, interactive window capture, repeat last capture, opening `.sss` documents, exporting the current screenshot, and Guide control/export. The app advertises common capture shortcuts for Apple Shortcuts and Spotlight, supports preset entities, maps output/file/format/delay/cursor/UI Map choices through shared validation, and uses foreground continuation for interactive workflows and UI-presenting outputs. | V1 App Intents do not expose annotation mutation, scrolling capture presets, connected-device capture, video recording, cloud upload, or direct window lookup by title/name. |
...
| Permission clarity | ✓ Done | Settings and Help explain Screen Recording, Guide-specific Accessibility, microphone/system-audio permissions, and which ordinary captures do not require Accessibility. The App Store build can initiate the Accessibility request for Guide, and the app exposes remediation actions plus support diagnostics export. | Keep troubleshooting copy aligned with macOS permission UI changes. |
...
| Release automation | ~ Partial | `FASTLANE.md`, project-local Fastlane lanes, and a GitHub App Store workflow cover doctoring, full-suite preflight, archive/upload, TestFlight, and review submission. The workflow no longer self-asserts metadata or manual QA: both are explicit dispatch confirmations, and the manual gate names signed-sandbox Guide, permission/restart, microphone, automation destination, and core smoke checks. | A production submission still depends on App Store Connect metadata/signing state and completion of the signed TestFlight checklist; the repo cannot prove those external checks automatically. |
