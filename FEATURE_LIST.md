...
| Recent Snips | ✓ Done | Shelved unsaved work remains available from the modeless Snip Library, which opens from both Capture and the editor inspector. Opening another non-private screenshot keeps the current non-private work here first. | No favorites, tags, or projects yet. |
| Snip History | ✓ Done | Archive-backed Snip History entries persist across sessions and support full store-backed search and paging. Capture shows quick preview thumbnails plus View All; the shared modeless Snip Library combines scopes, preserves window and browsing context, and provides a large zoomable preview without replacing editor content. | Filtering by type, date, and source app is still missing. |
...
| Recycle Bin | ✓ Done | Deleted entries move to the Recycle Bin and can be restored from the modeless Snip Library. Permanently Delete and Empty Recycle Bin require confirmation. New installs and Reset Defaults retain entries for 30 days; Settings > Snip Library > Snips accepts 1–180 days, with load-time and runtime clamping plus legacy-default migration. | Could add storage reporting. |
...
| Floating reference workflow | ✓ Done | Reference > Float Current Screenshot and the editor Float button create lightweight always-on-top views without duplicating files or modifying documents. The History Preview window can float an inspected snapshot for comparison or active reference work from its secondary actions. | Add saved reference layouts only if active workspace referencing becomes a larger product area. |
