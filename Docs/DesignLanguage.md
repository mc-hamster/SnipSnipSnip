...
- Customization saves immediately and therefore uses Close rather than Done. Restore Default Layout requires confirmation. Removing the final control leaves an intentional empty palette with a direct Customize action; it must not silently restore defaults or trap the final control in the layout.
