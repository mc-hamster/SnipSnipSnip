...
- Quick Controls is always available. Settings > General > Quick Controls exposes status, customization, and Show/Hide without an enablement gate. The main window's Screen Tools group, Capture menu, and menu bar icon use those same Show Quick Controls and Hide Quick Controls actions; hiding does not discard the saved layout. Main-window discovery pairs the button with a one-shot animation showing the edge dock expanding, and Reduced Motion shows its resolved expanded state.
