...
- The no-document header always pairs its action groups with a standard inset explanation stage. The main window uses a 1240-point no-document minimum and a 1280-point default so the stage is visible without manual resizing; document workflows retain their established 900-point minimum. Pointer hover updates the stage after a 50 millisecond dwell; keyboard focus updates it immediately; and the most recently explained action remains when the pointer leaves. Each explanation names the result or utility, states its practical outcome, and uses a detailed multi-element semantic scene. Every scene performs one short functional animation that demonstrates acquisition, transformation, recording, measurement, or state change; motion never loops. Reduced Motion uses the fully resolved static scene and a crossfade. The explanation stage is ordinary content chrome, not Glass or a promotional card.
