...
- The Capture workspace shows a small set of quick Snip History thumbnails, each with direct Preview and Open actions. **View All** and the editor's **Open Snip Library** action open one modeless **Snip Library** window with Recent Snips, Snip History, and Recycle Bin scopes, full store-backed search and paging, a stable list, and a large scrollable and zoomable preview. Selecting an item only inspects it; Open or Restore is the explicit commit point. Carry an active Capture search into View All. Reopening the window preserves its last scope, search, selection, divider, size, and position; first presentation centers on the invoking window's display.
- Opening or restoring a non-private screenshot while another non-private screenshot is open first keeps the current work in Recent Snips, then offers a brief **Undo** action that switches back without losing the newly opened screenshot. Do not dismiss a History Preview or Snip Library window until the requested screenshot has opened successfully. Private work retains the explicit Save, Discard, or Cancel decision because it cannot be kept in Recent Snips.
- Delete remains recoverable and needs no confirmation. Permanently Delete, Empty Recycle Bin, and the Settings action that permanently clears all Snip History always use a concise native confirmation that names the scope and states that the action cannot be undone.
...
- Introduce Quick Controls beside Launch at Login in the final setup step. Show its startup choice directly, default that choice on, and provide immediate Show/Hide and customization actions so setup is also a usable first encounter with the dock.
...
- **Quick Controls** is an optional desktop palette for a person's most-used SnipSnipSnip actions. It is an entry point into existing capture, creation, recording, clipboard, and screen-tool workflows; it must not reimplement those workflows or introduce alternate settings for them.
- Use a nonactivating floating panel for routine actions and a separate native titled window for customization. The palette follows the person across Spaces, remembers its visible screen frame, and restores into a connected display's visible frame when the display arrangement changes.
- The palette uses the approved floating-overlay surface. Expanded controls keep a visible label; Compact controls keep the same SF Symbol plus native help and accessibility labels. Every control retains focus behavior and a non-color-only state value. Use accent color only for active state. Hide routine Ready status; show labeled status only while capture, Guide, or Video activity changes availability.
- Quick Controls is content-sized edge chrome with two presentations: Expanded uses labeled rows and Compact uses an icon rail. Both retain the person's control order. Width follows the chosen presentation and height follows the configured controls automatically, capped only by the connected display's visible frame; neither dimension is directly resizable or separately configured. The dock snaps to the chosen left or right screen edge and preserves vertical placement. Do not expose freeform pixel placement or any width or height control.
- The header uses the application icon as its visual anchor and leaves non-button header space draggable; do not add a generic grip glyph that competes with the identity or implies a separate drag control. Expand and Collapse are permanent, edge-aware affordances rather than incidental header decoration. Compact pairs the brand mark with a distinct expand button pointing into the available screen area; Expanded points Collapse back toward its screen edge. Never make recovery from Compact depend on an ellipsis menu, an invisible window background, or remembering that the brand glyph is clickable.
- Customization uses a persistent searchable control library and an exact-size Dock Preview built from the same shell, section headers, control labels, and surface modifier as the live dock. The library and dock group Screenshot, Capture Options, Create, and Record so still-image and time-based actions never depend on icon shape or color alone. The library lists action and utility groups first and keeps the less-common Capture Options group at the bottom; this catalog order never rearranges the person's dock. Action controls use canonical verb-first labels such as Capture Region and Record Region; recording icons may add semantic red treatment as a redundant cue.
- A single control-state resolver, label, and surface modifier own the SF Symbol, menu indicator, supporting detail, active state, bounds, fill, border, and foreground treatment for both the live dock and Dock Preview. Supporting detail states the control's short practical intent or its current consequence; it never repeats the section name. Expanded and Compact are layout modes of those same primitives. Native Menu controls must use the same surface bounds as Button controls; hide the native menu indicator because the shared label supplies its own. Pin symbols to explicit rendering so Menu and Button environments cannot silently substitute variants. Customization-only selection and drag affordances sit outside the shared visual content.
- Library rows remain present with an Added state and a direct Remove action. Adding inserts after the current selection and scrolls it into view; drag-and-drop control reordering has visible Move Earlier and Move Later alternatives. A control drop uses the pointer's top or bottom half to choose before or after and shows a shaped accent insertion marker on that exact edge; never communicate an insertion solely by highlighting the destination control. Crossing into another section returns a forbidden drop proposal and marks the target with a red boundary plus prohibition symbol, while same-section targets retain the shared drag transport required for reliable reordering. In Dock Preview, each populated section header is its drag handle and the complete destination section is its drop target; moving a section retains its internal control order. Section headers expose Move Section Earlier and Move Section Later VoiceOver actions. Customization-only drag affordances may overlay shared preview headers without changing their structural size or the live dock renderer. A selected preview tile exposes a drag grip without turning the whole customization surface into custom chrome. Preview tiles remain native buttons with keyboard focus and VoiceOver actions.
- Customization saves immediately and therefore uses Close rather than Done. Restore Defaults… is a visible labeled action in the customization header, separated from Show/Hide and Close; never bury it in the muted preview editor beside contextual actions that may be disabled. It requires confirmation and restores the first-install control set, order, Expanded presentation, and right screen edge without changing visibility. Arrangement guidance belongs beside Dock Preview, and the preview editor uses a normal adaptive window background so selected-control and Dock Settings groups read as interactive. Removing the final control leaves an intentional empty palette with a direct Customize action; it must not silently restore defaults or trap the final control in the layout.
- Set the palette panel's sharing type to none and continue routing display capture through the existing SnipSnipSnip process-exclusion filters. Window discovery must continue excluding the current process. This guarantees exclusion from captures made by SnipSnipSnip; Help must not promise exclusion from unrelated capture applications.
- Quick Controls is always available and appears when SnipSnipSnip starts by default. **Show Quick Controls when SnipSnipSnip starts** is an opt-out startup preference, not an enablement gate. Expose it in onboarding, Settings > General > Quick Controls, and Dock Settings in customization. Settings also exposes current status, customization, and Show/Hide. The main window's Screen Tools group, Capture menu, and menu bar icon use those same Show Quick Controls and Hide Quick Controls actions; hiding does not discard the saved layout or change the startup preference. Main-window discovery pairs the button with a one-shot animation showing the edge dock expanding, and Reduced Motion shows its resolved expanded state.
- Quick Controls preserves interaction context. Expanding, collapsing, and opening a utility stay on the dock's current display. Region and on-screen Window selection begin without activating the main window; Cancel returns to the previously active app with the dock still available. A successful capture may open its editor, while permission setup and recoverable errors may open the main window because they require attention.

### Main window and editors

- Capture Text is a direct Quick Capture action and menu/shortcut command. Reuse Region selection and local OCR without installing a screenshot, prompting about the open document, or saving captured pixels. Confirm successful clipboard delivery without opening the editor. Empty recognition leaves the clipboard unchanged. In the editor, Copy Text preserves line structure and offers explicit Join Lines cleanup.
- Highlighter's Properties expose a native Fit to Text toggle, on for a new editor session. Show the freehand gesture immediately; fit confident word/line geometry asynchronously only while that stroke remains the latest unchanged edit. Fitting and the original gesture share one Undo step. Uncertain recognition never removes the drawn stroke.
- A native Size popover sits in the screenshot Output command group beside Copy, Export, and Share. Offer Original Size, Half Size, and Custom Width with linked height, pixel dimensions, validation, and an explicit Apply Width action. Explain that this session-only setting changes delivered still images, not editable originals. Reuse the same final raster resize for Copy, Share, Drag, and still-file export; animation, HTML, history, and automation keep their established sizing.

- Ordinary standalone screenshots open in the editor immediately by default, keeping annotation and output actions directly available. **Capture Preview** is an opt-in alternative for non-private screenshots: a nonactivating lower-left panel on the pointer's display, inside its visible frame. The thumbnail is the rendered output, including any captured cursor, and supports drag-out or click-to-edit. Keep Copy prominent beside Edit and Export, with visible drag guidance and truthful completion/error status. Do not auto-dismiss, steal application focus, or delete work when Close is chosen. Show Capture Preview in View and the menu bar icon menu restores the presentation until editing or replacement invalidates it. Reuse existing document, recovery, rendering, and promised-file output services; do not create a second screenshot store. Exclude the panel from this app's capture sources. Private Capture, explicit presets, automation, connected devices, and composition acquisition keep their existing destinations. Settings > Editor & Output > After Capture owns the opt-in preference; preserve stored choices and turn the preference off when restoring defaults.
- Auto Copy is opt-in when no preference is stored; preserve stored choices. Explain that enabling it can copy a capture before redaction. Manual Copy confirms completion only after the clipboard write succeeds. The common-tool Redact action always starts solid redaction; keep Blur and Pixelate accessible through its menu. Show concise guidance that Copy and Export apply redactions while editable files retain original pixels. Copy remains the prominent completion action in the screenshot command row.

- Capture is the main window's persistent workflow, so its primary actions belong in a compact in-content header rather than the title-bar toolbar. With no document open, the adaptive header keeps the product name and textual readiness status above four labeled groups: **Quick Capture** exposes Region, Window, Screen, Scroll when available, Repeat Last, and a Presets menu; **Create** exposes Comparison, Steps, and Combined Image as individually labeled one-click setup entries; **Record** exposes Region, Window, Screen, Guide when available, and connected devices as direct choices; and **Screen Tools** exposes Screen Ruler, Screen Inspector, and Quick Controls beside direct Clipboard History access. Quick Capture actions share the same native bordered treatment so accent color does not imply a persistent selection. Region, Window, Screen, Scroll, and Repeat Last are direct one-action Screenshot paths; Presets and Screen Ruler remain menus because they contain variable presets or horizontal and vertical ruler choices. Each creation entry opens setup with its result already selected; each recording-source entry starts its focused selection or recording path without an intermediate Record menu. Keep Timer, Cursor, Private Capture, and Auto Copy state visible beside the header identity so capture-affecting state is not hidden in Settings or menus. Keep the live-window carousel visible in the main content because its direct thumbnails are a unique one-click Window capture path rather than additional setup.
...
- History thumbnails open one modeless **History Preview** auxiliary window without replacing or dimming the editor canvas. Reuse that window as people move among neighboring Change History, Recent Snips, Snip History, or Recycle Bin items; show the neutral Screenshot title, source, position, and saved time above a scrollable and zoomable full preview. Provide Fit, Actual Size, direct zoom controls, keyboard navigation, standard window closing, and a source-correct primary action only when applicable: Restore for Change History, Recent Snips, and Recycle Bin, or Open for Snip History. Keep Float Reference in secondary actions because it creates a separate always-on-top reference rather than changing the preview window's role. The Existing Image picker follows the same inspect-before-commit pattern with a two-pane browser and large zoomable preview; **Use Screenshot** preserves editability by default, while **Use Rendered Image Only** is an optional explicit choice.
- Preserve spatial memory in the screenshot editor with two stable in-content command rows. The first row starts with Discard or Cancel; Select, Crop, the Arrow family, Text, Highlight Box, and solid Redact remain labeled one-click controls in stable positions in the common-tool group. The Arrow split control contains Arrow and Numbered Arrow and may contain additional arrow variants in the future. Less-common tools use four additional labeled split controls: Shapes, Draw, Emphasize (Spotlight and Ruler), and More Tools. Redact keeps a separate adjacent menu for Blur, Pixelate, and Redact; its main action always starts solid Redact rather than recalling a weaker concealment mode. Each split control has a visible disclosure-arrow target beside its main button. Except for the fixed solid Redact action, the main button's text and icon identify the last-selected member and activate that member directly; opening the disclosure menu exposes every named variant in one additional step. The second row keeps History, Layers and Arrangement, Zoom, Inspector, Workspace, Output, and References and Drag Out as distinct visual groups in workflow order without a separating expanse of empty space.
...
- Video keeps Trim and Polish visible beside the preview and detailed controls in a native trailing inspector (280/320/380 points, remembered visibility). Use plain-language section labels, a one-action Polish Video starting point, and disclosures for fine timing and motion. Every edit is non-destructive and undoable; slider drags form one undo step. Keep metadata outside media pixels, successful export feedback inline, and preview preparation nonmodal. Use the shared presentation renderer for preview, posters, and every export format.
- Video Export opens a native sheet with format and quality, optional MP4 file-size limits, then Choose Location. Explain that animated loops omit sound before export. File > Export and drag-out retain the last selected options.
...
- Settings uses a searchable native navigation sidebar with the existing nine categories (Guide remains capability-gated). Search matches category names and intent keywords; details remain ordinary grouped forms. Do not duplicate settings or introduce a second terminology for the same control.
...
| --- | --- | --- |
| Capture Preview — `Capture/CapturePreviewView.swift` | Provides transient post-capture output actions above another app without opening the editor | Uses `sssFloatingOverlaySurface` with adaptive opaque/high-contrast/reduced-motion behavior; labeled native actions, keyboard focus on interaction, Escape to close, explicit completion/error text, and no automatic timeout |
...
| Quick Controls palette — `App/QuickControls/QuickControlsPaletteView.swift` and `QuickControlsComponents.swift` | Keeps a configurable set of existing app actions available above other apps without duplicating their workflows | Uses `sssFloatingOverlaySurface`; every tile has a visible label and help text, active state uses label plus boundary in addition to accent color, and the native customization window remains ordinary adaptive app chrome |
...
| Video player and export overlays — `Video/VideoEditorView.swift` | A dark content stage prevents unused player area from competing with video; cancellable export progress temporarily covers it | Playback metadata stays outside media; overlay panels use the opaque/high-contrast/reduced-motion fallback; controls and inspectors use native adaptive chrome |
